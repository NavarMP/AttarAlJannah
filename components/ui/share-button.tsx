...
export function ShareButton({ data = defaultShareData, variant = "default", size = "default" }: ShareButtonProps) {
    const [copied, setCopied] = useState(false);

    const handleCopyLink = async () => {
        const success = await copyLink(data.url);
        if (success) {
            setCopied(true);
    ...
    const handleNativeShare = async () => {
        ...
        if (!success && !isNativeShareAvailable()) {
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem onClick={() => shareToWhatsApp(data)}>
