...
export async function getLeaderboardData(metric: string = "overall", period: string = "all", limit: number = 100) {
...
export async function getVolunteerRank(volunteerId: string, metric: string = "overall", period: string = "all") {
    // We fetch all to guarantee we find the rank accurately even if it's > 100
    const fullLeaderboard = await getLeaderboardData(metric, period, 0);
    const entry = fullLeaderboard.find((v: any) => v.volunteer_id.toLowerCase() === volunteerId.toLowerCase());
